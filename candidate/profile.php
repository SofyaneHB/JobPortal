...
<head>
    ...
    <title>Mon Profil | Sofyane_HB</title>
    ...
    <main class="ml-64 flex-1">
        <nav class="border-b border-slate-900 bg-slate-950/40 backdrop-blur-md sticky top-0 z-30">
            <div class="flex items-center justify-between px-8 py-3">
                <div class="text-xs text-slate-500 font-medium">Mon Profil</div>
        ...
        <div class="p-8 md:p-12 max-w-3xl">
            <div class="mb-8">
                <div class="text-[11px] font-bold uppercase tracking-wider text-emerald-400 mb-1">Profil</div>
                <h1 class="text-2xl md:text-3xl font-bold tracking-tight text-white">Mon Profil</h1>
            </div>
            ...
            <div class="bg-slate-900/20 border border-slate-900 rounded-2xl p-6 md:p-8">
                <form method="POST" action="update_profile.php" class="space-y-5">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                        ...
                        <div>
                            <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Pays</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Adresse</label>
                        <textarea name="address" rows="2" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-200 focus:border-emerald-500 transition"><?= htmlspecialchars($user['address'] ?? '') ?></textarea>
                    </div>
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Compétences</label>
                        <textarea name="skills" rows="3" class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-200 focus:border-emerald-500 transition"><?= htmlspecialchars($user['skills'] ?? '') ?></textarea>
                    </div>
