...
<head>
    ...
    <title>Profil Entreprise | Sofyane_HB</title>
...
<body class="flex bg-slate-950 text-slate-100 min-h-screen selection:bg-indigo-500 selection:text-white">
    ...
    <aside class="w-64 border-r border-slate-900 bg-slate-950/40 backdrop-blur-md h-screen fixed flex flex-col justify-between z-40">
        <div>
            ...
            <nav class="p-4 space-y-1.5 text-xs font-medium">
                ...
                <a href="profile.php" class="flex items-center px-3 py-2.5 bg-indigo-600/10 border border-indigo-500/20 text-indigo-400 rounded-xl font-semibold">Profil Entreprise</a>
    ...
    <main class="ml-64 flex-1 p-8 md:p-12 max-w-4xl">
        ...
        <!-- Header -->
        <div class="mb-8">
            <div class="text-[11px] font-bold uppercase tracking-wider text-indigo-400 mb-1">Identité de marque</div>
            <h1 class="text-2xl md:text-3xl font-bold tracking-tight text-white">Profil Entreprise</h1>
            <p class="text-slate-400 text-xs md:text-sm mt-1">Mettez à jour les informations et le branding de votre entreprise pour attirer les meilleurs profils.</p>
        </div>
        ...
        <div class="bg-slate-900/20 border border-slate-900 p-6 md:p-8 rounded-2xl backdrop-blur-sm">
            <form method="POST" action="" class="space-y-6">
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Nom de l'entreprise *</label>
                    ...
                    <div>
                        <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Logo URL (Lien de l'image)</label>
                ...
                <div>
                    <label class="block text-xs font-semibold uppercase tracking-wider text-slate-400 mb-2">Description de l'entreprise</label>
                    <textarea name="description" rows="5" placeholder="Présentez votre entreprise, votre culture technique (Agile, DevOps) et vos secteurs d'activité..." 
                              class="w-full bg-slate-950 border border-slate-800 rounded-xl px-4 py-3 text-sm text-slate-200 placeholder-slate-600 focus:outline-none focus:border-indigo-500 transition leading-relaxed"><?= htmlspecialchars($company['description'] ?? '') ?></textarea>
                </div>
